...
<template>
  <div class="portfolio">
    <transition name="fade">
      <div
        v-if="loading == true || mobileDetected == true"
        class="portfolio-loading"
      >
        <h1 v-if="mobileDetected != true">Loading...</h1>
        <h1 v-if="mobileDetected == true">
          Mobile Phone Detected! Please use a computer
        </h1>
      </div>
    </transition>
...
export default {
  ...
  mounted() {
    ...
    console.log(this.mobileDetected)
    if (this.mobileDetected == false) {animate();}
